...
public:
	...
	void OperatorControl(){
		while(IsOperatorControl() && IsEnabled()){
			...
			double Throttle 	= 	OpController.GetRawAxis(1);
	...
	// -----Begin block of drive code
		if (Brake){		//brake, Bracket level 1
			Left = 0;
			Right = 0;
			} else  (Slew > Thresh){	//If Slew is positive (Thumbstick pushed right), go Right, new bracket L2
	...
	//------End block of spin in place code
			...
			SetMotors(LeftDriveDesired, RightDriveDesired);
